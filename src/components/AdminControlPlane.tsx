...
type Detail = { session: InterviewSession; runs?: Run[]; evaluations?: Evaluation[]; operatorNotes?: OperatorNote[]; confirmedAt?: string | null };
...
export function AdminControlPlane() {
  ...
  const generateDemoPrompt = async () => {
    ...
    try {
      const payload = await request('/api/admin/session-demo-prompt', 'POST', { sessionId: detail.session.id }) as { demoId: string; prompt: string; confirmed: boolean };
      const url = URL.createObjectURL(new Blob([payload.prompt], { type: 'text/markdown' }));
      const link = document.createElement('a'); link.href = url; link.download = `mirai-demo-prompt-${detail.session.id.slice(0, 8)}.md`; link.click(); URL.revokeObjectURL(url);
      setNotice(payload.confirmed
        ? `Prompt demo pobrany. Zarejestrowano jako ${payload.demoId} w sekcji "Demo dla klientów".`
        : `Prompt demo pobrany (UWAGA: wynik jeszcze niepotwierdzony przez uczestnika — prompt to jawnie zaznacza). Zarejestrowano jako ${payload.demoId} w sekcji "Demo dla klientów".`);
