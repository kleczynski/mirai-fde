...
## Doprecyzowanie wykonawcze, 2026-09-20

Decyzja ratyfikowana przez Astrę w granicach autonomii operatora. Historyczny status Accepted oznacza zatwierdzenie projektu, nie dowód ukończenia; stan budowy jest w `docs/scope/scope.md`.

### Wybór i alternatywy

Zachowujemy osobny Worker z Workflow i wspólnym Supabase. Prosty synchroniczny endpoint byłby krótszy, ale nie utrzyma pięciu etapów ani oczekiwania na admina. Własna maszyna Durable Object powielałaby mechanizmy Workflow. Nowe tabele mają RLS bez polityk dla uczestników i dostęp wyłącznie przez zweryfikowany backend administratora, tak jak monitoring Node 1.

Workflow przenosi tylko runId i kontrolne wyniki etapów. Każdy etap wczytuje wejście z Supabase, zapisuje tam zredagowane wejście i wyjście, a do checkpointu nie zwraca treści. Zapobiega to zachowaniu dowodów po usunięciu sesji w historii Cloudflare. Otwarte oczekiwanie na review trwa maksymalnie 30 dni; API oraz każdy etap sprawdzają wcześniejszą datę wygaśnięcia sesji. Treści mają FK z ON DELETE CASCADE. Osobny licznik kosztów zachowuje tylko kwoty i anonimowe identyfikatory wywołań, bez tekstu i identyfikatora klienta, aby usunięcie danych nie resetowało budżetu.

### Źródła wartości i API

Eksport Node 1 uzupełniamy addytywnie o evidence, painPoints i revision. Wcześniejszy eksport pomijał te dane, choć ADR zakładał sprawdzenie powiązań dowodów. Worker przyjmuje wyłącznie sessionId i requestId, sam weryfikuje JWT przez Supabase Auth i allowlistę oraz wczytuje ukończony, niewygasły raport. Nie przyjmuje od przeglądarki podmienionych approvedFacts. Wersję przypina hash zredagowanego kontekstu, obejmujący revision; zatwierdzanie i eksport ponownie porównują źródło.

| Operacja | Wejście | Wynik i źródło |
| --- | --- | --- |
| POST /runs | sessionId, requestId UUID | runId=requestId, zapisany snapshot, status queued; ponowienie zwraca ten sam run, konflikt innej sesji daje 409 |
| GET /runs?sessionId=… | sessionId, limit do 25, opcjonalny cursor | ostatnie uruchomienia z Supabase |
| GET /runs/:id | runId | zapisane kroki, finalny brief, quality, koszt i status |
| POST /runs/:id/review | hash briefu, decyzja approve albo reject | tożsamość z Auth, czas serwera, zatwierdzenie tej wersji albo failed |
| POST /runs/:id/export | format json albo markdown | tylko zatwierdzony aktualny brief, czas eksportu serwera |

Panel używa pojedynczego proxy `/api/admin/brief-runs` z walidowaną akcją. UI korzysta z istniejących kart, przycisków, stanów ładowania i błędów. Treść modelu wyświetla jako tekst, bez HTML. Review briefu nie uprawnia do udostępnienia demo klientowi ani wdrożenia produktu klienta.

### Płatne wywołania i jakość

Model `gpt-4.1-mini`, zgodny z domyślnym Node 1. Cena przyjęta do górnego ograniczenia: 0,40 USD za milion tokenów wejścia oraz 1,60 USD za milion wyjścia. Maksymalnie 60000 bajtów wejścia łącznie ze schematem i instrukcjami oraz 6000 tokenów wyjścia na call. Rezerwujemy konserwatywnie 0,04 USD przed każdym call, maksymalnie 6 call (5 etapów i jedna poprawka), 0,24 USD/run i 1,48 USD dla Node 2 (zwiększenie testów o 0,48 USD zatwierdzone przez operatora 2026-09-20). Brak automatycznych retry OpenAI, fallbacku modelu i zwalniania niepewnej rezerwacji. Globalna blokada SQL serializuje przydział budżetu. Budżet 1,48 USD dzielimy konserwatywnie: 0,96 USD na lokalne evale (trwały plik `.local/brief-studio/eval-budget.json`, bez resetowania) i 0,52 USD na Workflow. Migracja od razu rezerwuje pełne 0,96 USD ewaluacji. CI odtwarza zapisany wynik prawdziwego OpenAI i weryfikuje hash promptów, schematów, klienta API i fixture; zmiana tych źródeł wymaga świeżego evala. CI samo nie uruchamia płatnych call na każdym pushu. Bramka evala ocenia końcowy brief dodatkowym, niezależnym wywołaniem krytyka po poprawce i sprawdza kalibrację na celowo niebezpiecznej wersji (realne konta i przelew). Obie oceny kosztują najwyżej po 0,04 USD z alokacji testów; pakiet evala to maksymalnie 0,32 USD (pipeline do 0,24 USD plus dwa testy jakości). Runtime nadal ma pięć etapów i najwyżej jedną poprawkę; wcześniejsza krytyka pozostaje widoczna. Release wymaga braku blokad końcowego wyniku oraz wykrycia blokady w celowo wadliwym przykładzie. To zastępuje ocenianie releasu wyłącznie po szkicu: pomijałoby efekt poprawki i nadawało fałszywym alarmom autorytet bez testu kalibracji. Limity to granice aplikacji, nie całego konta OpenAI; zmiana cen/modelu wymaga nowej decyzji.

Deterministyczny evaluator sprawdza pochodzenie cytatów i identyfikatorów, jedną okazję, komplet odrzuconych alternatyw, dane fikcyjne, brak autoryzacji wygenerowanej przez model, stałe instrukcje i konkretne przykłady błędów. Krytyk semantyczny to obowiązkowe piąte wywołanie. Jedna poprawka nie usuwa pierwotnej krytyki i nie oznacza automatycznie rozwiązania uwag. Operator widzi niezweryfikowane poprawki. Eval CI sprawdza fixture Julki i przypadki negatywne; dowód rzeczywistego provider run jest dodatkową wymaganą przesłanką releasu, nie zastępuje go mock ani sam parser Zod.

### Kryteria odbioru

Kalibracja 2026-09-20: trzy rzeczywiste przebiegi pokazały, że `gpt-4.1-mini` jako sędzia fałszywie blokuje jawnie opisane symulacje i hipotezy. Zachowujemy go w runtime jako pomocniczego krytyka przed przeglądem człowieka. Niezależna bramka releasu używa `gpt-4.1`, z rezerwacją 0,08 USD na ocenę, maksymalnie 20000 bajtów wejścia i 2000 tokenów wyjścia (2/8 USD za milion tokenów według oficjalnego cennika). Dwie oceny kosztują maksymalnie 0,16 USD z już zatwierdzonej alokacji 0,96 USD. Nie generujemy ponownie niezmienionego briefu: ponowna ocena weryfikuje zapisane hashe źródeł generacji. Zachowujemy wcześniejsze wyniki. Limit runtime nadal 0,24 USD; dodatkowy sędzia działa tylko w lokalnym eval, nie w produkcyjnym Workflow.

- AC-1: pełne pięć typowanych etapów, najwyżej jedna poprawka, sensowny brief Julki ze ścieżką błędu.
- AC-2: brak dostępu anonimowego i spoza allowlisty, podmiany wejścia, wycieku sekretów lub uznania danych za instrukcje.
- AC-3: brak podwójnego płatnego wywołania przy replay, atomowy limit, zachowanie rezerwacji po timeout i usunięciu danych.
- AC-4: panel pokazuje status, wejścia, wyjścia, krytykę i koszty; review oraz eksport dotyczą dokładnej aktualnej wersji.
- AC-5: usunięcie/wygaśnięcie źródła uniemożliwia odczyt i dalszą generację, treści usuwają się kaskadowo.
- AC-6: CI obejmuje Worker, migracje testowane w Postgres, e2e i quality gate, a deploy/migracja zależą od wszystkich checków.

Źródła sprawdzone 2026-09-20: [zasady Workflows](https://developers.cloudflare.com/workflows/build/rules-of-workflows/), [retencja checkpointów](https://developers.cloudflare.com/workflows/reference/limits/), [Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs), [model i ceny](https://developers.openai.com/api/docs/models/gpt-4.1-mini).
